...
coords_ = cbind(spat.stat.linnet$vertices$x-mu_x, 
                spat.stat.linnet$vertices$y-mu_y)
Windows_ = owin(xrange=c((min(spat.stat.linnet$vertices$x)-mu_x),
                         (max(spat.stat.linnet$vertices$x)-mu_x)), 
                yrange=c((min(spat.stat.linnet$vertices$y)-mu_y),
                         (max(spat.stat.linnet$vertices$y)-mu_y))
)
...
locs = cbind(locs[,1]-mu_x, locs[,2]-mu_y)
