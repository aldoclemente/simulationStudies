...
In questo esempio le istruzioni presenti nel chunk **NON** sono eseguite e sono stampate sul file pdf. \newline
...
In questo esempio le istruzioni presenti nel chunk sono eseguite e **NON** stampate nel file pdf.
In particolare, `echo=FALSE` evita che le istruzioni inserite nel chunk vengano stampate sul file pdf  e `results="hide"` evita che l'output che osserveresti su console venga stampato sul file pdf.
...
All'interno del chunk, assicurarsi di non superare la barra verticale posta sulla destra. Questo accorgimento permette che codice stampato nel file output non venga troncato risultando illegibile, come mostrato dalla figura che segue.
...
Per approfondimenti su `RMarkdown` fare riferimento a [R Markdown Cookbook](https://bookdown.org/yihui/rmarkdown-cookbook/).

##### Shortcuts da tastiera per tilde e back tick (utenti windows) \newline
Di seguito le istruzioni per creare shortcuts da tastiera per la **~** **(tilde)** (combinazione `AltGr+'`) e **`** **(back tick)** (combinazione `AltGr+ì`)
